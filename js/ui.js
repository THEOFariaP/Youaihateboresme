...
class UIManager {
  ...
  cacheElements() {
    ...
    // -- Welcome screen -------------------------------------------------------
    this.elUsernameInput = document.getElementById("username-input");
    this.elBtnCreateRoom = document.getElementById("btn-create-room");
    this.elBtnJoinRoom = document.getElementById("btn-join-room");
    this.elJoinRoomGroup = document.getElementById("join-room-group");
    this.elRoomCodeInput = document.getElementById("room-code-input");
    this.elBtnConnectRoom = document.getElementById("btn-connect-room");
  ...
  initEventListeners() {
    // -- Welcome: Create Room -------------------------------------------------
    if (this.elBtnCreateRoom) {
      this.elBtnCreateRoom.addEventListener("click", () => {
        const username = this._getValidatedUsername();
        if (username === null) return;
        this.emit("create-room", username);
      });
    }

    // -- Welcome: Join Room (toggle join group) -------------------------------
    if (this.elBtnJoinRoom) {
      this.elBtnJoinRoom.addEventListener("click", () => {
        if (this.elJoinRoomGroup) {
          this.elJoinRoomGroup.classList.toggle("hidden");
          // Focus the room code input when revealed
          if (
            !this.elJoinRoomGroup.classList.contains("hidden") &&
            this.elRoomCodeInput
          ) {
            this.elRoomCodeInput.focus();
          }
        }
      });
    }

    // -- Welcome: Connect to room ---------------------------------------------
    if (this.elBtnConnectRoom) {
      this.elBtnConnectRoom.addEventListener("click", () => {
        this._handleConnectRoom();
      });
    }
    ...
    this._bindEnterKey(this.elUsernameInput, () => {
      // If join group is visible, do nothing — user likely wants to type room code
      if (
        this.elJoinRoomGroup &&
        !this.elJoinRoomGroup.classList.contains("hidden")
      ) {
        return;
      }
      const username = this._getValidatedUsername();
      if (username !== null) this.emit("create-room", username);
    });

    this._bindEnterKey(this.elRoomCodeInput, () => {
      this._handleConnectRoom();
    });
  ...
  /**
   * Validate username input and return trimmed value, or null if invalid.
   * @returns {string|null}
   */
  _getValidatedUsername() {
    if (!this.elUsernameInput) return null;
    const val = this._sanitizeInput(this.elUsernameInput.value, 20, false);
    if (!val) {
      this.showNotification("Please enter a nickname.", "warning");
      this.elUsernameInput.focus();
      return null;
    }
    this.elUsernameInput.value = val;
    return val;
  }

  /** Handle "Connect" button press in the welcome join group. */
  _handleConnectRoom() {
    const username = this._getValidatedUsername();
    if (username === null) return;

    if (!this.elRoomCodeInput) return;
    const roomCode = this._sanitizeRoomCode(this.elRoomCodeInput.value);
    if (!roomCode) {
      this.showNotification("Please enter a valid room code.", "warning");
      this.elRoomCodeInput.focus();
      return;
    }

    this.elRoomCodeInput.value = roomCode;
  ...
  onGameEnd(scores, reason) {
    ...
    // Return to welcome after a brief pause so the user can read the toast
