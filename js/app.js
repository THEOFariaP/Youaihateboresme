...
function wireEvents(network, ai, game, ui) {
  ...
  function maybeAutoStartGame() {
    const canAutoStart =
      network.isHost() &&
      network.getPlayers().length >= 3 &&
      ai.isConfigured() &&
      game.state === "lobby";

    if (!canAutoStart) return;

    ui.showNotification("3+ players connected. Starting automatically...", "info", 1800);
  ...
  async function autoConnectGlobal() {
    ...
    try {
      ui.showLobby(GLOBAL_ROOM_CODE);
      ui.updateConnectionStatus("connecting");

      const result = await network.connectAutoGlobal();
      const roomCode = (result && result.roomCode) || GLOBAL_ROOM_CODE;
      ...
      if (role === "host") {
        ui.showNotification("Connected to global room as host.", "success");
      } else {
        ui.showNotification("Connected to global room.", "success");
      }
    } catch (err) {
      console.error("[App] Auto-connect failed:", err);
      ui.updateConnectionStatus("error");
      ui.showNotification(
        "Could not auto-connect. Retrying in 2 seconds...",
        "warning",
      );
  ...
  network.on("disconnected", () => {
    ui.showNotification("Disconnected. Reconnecting automatically...", "warning");
    ui.showLobby(GLOBAL_ROOM_CODE);
    autoReconnectTimer = setTimeout(() => {
      autoConnectGlobal();
    }, 1200);
