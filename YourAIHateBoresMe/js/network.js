...
class NetworkManager {
  ...
  async createRoom(playerName) {
    ...
    const cleanName = this._sanitizeName(playerName);
    if (!cleanName) {
  ...
  async joinRoom(roomCode, playerName) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.peer.on("open", async (id) => {
        ...
        try {
          const candidates = this._getCandidateHostIds();
  ...
  _beginHostTakeover() {
    ...
    const nextHostId = this._computeNextHostId();
    if (!nextHostId) {
      this.emit(
        "error",
    ...
    this._scheduleReconnectToReplacementHost(nextHostId);
  }
  ...
  _scheduleReconnectToReplacementHost(nextHostId) {
    this._clearReconnectTimer();
    this.emit("connection-status", "connecting");

    this.reconnectTimer = setTimeout(async () => {
      try {
        await this._connectToSpecificHost(nextHostId);
        this.hostMigrationInProgress = false;
        this.emit("host-changed", {
        ...
        console.warn("[Network] Reconnect to replacement host failed:", err);

        // If the intended replacement also failed, recompute again from current knowledge.
        this.hostMigrationInProgress = false;
        this._beginHostTakeover();
      }
